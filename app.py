...
from flask import Flask, json, render_template, request , session, redirect,url_for
import sqlite3
...
def search():
    search_query = request.args.get('search_blog')
    conn = sqlite3.connect('blog.db')
    cursor = conn.cursor()
...
@app.route('/view-blog/<int:blog_id>')
def view_blog(blog_id):
    ...
    conn.commit()
    conn.close()
    return render_template('view-blog.html',blogs=blogs,comments=comments)
...
def edit_blog(blog_id):
    ...
    blogs = cursor.fetchone()
    conn.close()
    return render_template('edit-blog.html',blogs=blogs)
...
def comment():
    ...
    conn = sqlite3.connect('blog.db')
    cursor = conn.cursor()
    cursor.execute(''' INSERT INTO comments(blog_id,comment,user_id) VALUES(?,?,?) ''',(blog_id,comment,user_id) )
    conn.commit()
    conn.close()
...
def form_insert():
    ...
    if action == 'generate':
        ...
        return render_template('new-blog.html',title=title,image=image,description=generated_description)
    ...
    if action == 'submit':
        ...
        cursor.execute(''' INSERT INTO blogs(title,image,description,author,date,user_id) 
                    VALUES(?,?,?,?,?,?) ''', 
...
def generate_blog(title):
    prompt = f"write a detailed blog description about {title} in around 200 - 300 words"
...
def update_blog(blog_id):
    ...
    cursor.execute(''' UPDATE blogs SET title = ? , image = ? , description = ? , author = ? , date = ? WHERE id = ?''', 
